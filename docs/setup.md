...
# Review the diff - brew bundle dump includes everything currently installed
```

Note: `[add] secrets = "error"` is configured to prevent accidentally adding unencrypted secrets.
...
Oh-My-Zsh and zsh plugins are managed via `.chezmoiexternal.toml` (auto-downloaded as archives from GitHub, refreshed weekly). This replaces brew-installed zsh plugins and manual oh-my-zsh installation.

## Bootstrap Scripts

Scripts in `.chezmoiscripts/` handle Homebrew install, package updates, and macOS settings. All scripts are templated to only run on macOS.
