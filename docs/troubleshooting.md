...
- Missing cargo/rust env file (guarded with existence check)
...
**Symlink conflicts** (e.g., gping):
```bash
brew link gping --overwrite
```

## Zsh Plugins Not Found

**Symptoms**: `plugin 'zsh-autosuggestions' not found` or similar on shell startup

**Fix**: Plugins are managed via `.chezmoiexternal.toml` and installed into `~/.oh-my-zsh/custom/plugins/`. Run:
```bash
chezmoi apply
```

If plugins are still missing, force re-download:
```bash
chezmoi state delete-bucket --bucket=entryState
chezmoi apply
```

## chezmoi Lock File Issues

**Symptoms**: `chezmoi apply` hangs or errors about database lock

**Fix**:
```bash
# Check for stuck chezmoi processes
pgrep -f chezmoi
